...
async function listAllStrapiFiles() {
  try {
    ...
    if (response.ok) {
      ...
      // Group files by track patterns to help identify all available tracks
      ...
      console.error(`[DEBUG] Failed to fetch files: ${response.status} ${response.statusText}`);
      return [];
    }
  } catch (error) {
    console.error(`[DEBUG] Error listing files:`, error);
    return [];
  }
}
